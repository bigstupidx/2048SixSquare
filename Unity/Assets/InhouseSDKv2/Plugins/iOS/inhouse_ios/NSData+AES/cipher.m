...
NSData *sha1(NSData *bytes) {
  ...
  if (result != CC_SHA1([bytes bytes], (CC_LONG)[bytes length], result)) {
    [NSException exceptionWithName:@"SHA1Exception"
                                   reason:@"Unknown"
                                 userInfo:nil];
  }
  return _md;
...
NSData *sha256(NSData *bytes) {
  ...
  if (result != CC_SHA256([bytes bytes], (CC_LONG)[bytes length], result)) {
    [NSException exceptionWithName:@"SHA256Exception"
                                   reason:@"Unknown"
                                 userInfo:nil];
  }
  return _md;
...
NSData *cipher(NSData *key,
  ...
  switch (ccStatus) {
    ...
    case kCCParamError:
      [NSException exceptionWithName:@"IllegalParameterValueException"
                                     reason:@"Illegal parameter value."
                                   userInfo:nil];
      break;
    case kCCBufferTooSmall:
      [NSException exceptionWithName:@"InsufficentBufferException"
                                     reason:@"Insufficent buffer provided for specified operation."
                                   userInfo:nil];
      break;
    case kCCMemoryFailure:
      [NSException exceptionWithName:@"MemoryAllocationFailure."
                                     reason:@"Memory allocation failure."
                                   userInfo:nil];
      break;
    case kCCAlignmentError:
      [NSException exceptionWithName:@"InputAlignmentException"
                                     reason:@"Input size was not aligned properly. "
                                   userInfo:nil];
      break;
    case kCCDecodeError:
      [NSException exceptionWithName:@"DecryptionException."
                                     reason:@"Input data did not decode or decrypt properly."
                                   userInfo:nil];
      break;
    case kCCUnimplemented:
      [NSException exceptionWithName:@"FunctionNotImplementedException"
